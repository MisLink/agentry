...
function createTestReviewContextTool(context: ReturnType<typeof buildReviewContext>) {
	return createReviewContextTool(() => context, {
		name: "review_context",
		label: "Review Context",
		description: "Read-only diff/file lookup for active review sessions.",
	});
}

test("buildReviewContext keeps diff snapshot, changed files, per-file diffs, metadata, and hunks", () => {
...
test("review context tool returns diff snapshot, file contents, discovery info, metadata, and excerpts", async () => {
	const tool = createTestReviewContextTool(buildReviewContext({
	...
	const diffResult = await tool.execute("tool-1", { kind: "diff" });
	const fileResult = await tool.execute("tool-2", { kind: "file", path: "src/auth.ts" });
	const listFilesResult = await tool.execute("tool-3", { kind: "list-files" });
	const fileDiffResult = await tool.execute("tool-4", { kind: "file-diff", path: "src/auth.ts" });
	const fileMetaResult = await tool.execute("tool-5", { kind: "file-meta", path: "src/old.ts" });
	const excerptResult = await tool.execute("tool-6", { kind: "file-excerpt", path: "src/auth.ts", startLine: 2, endLine: 3 });
...
test("review context tool can expose current shared context under a custom tool name", async () => {
	const tool = createReviewContextTool(() => buildReviewContext({
		diffSnapshot: "shared diff snapshot",
		files: [{ path: "src/review.ts", content: "export const ok = true;" }],
		fileMetadata: [{ path: "src/review.ts", state: "available", lineCount: 1 }],
	}), {
		name: "review_context",
		label: "Review Context",
		description: "Read-only shared review context.",
	});

	const listFilesResult = await tool.execute("tool-shared", { kind: "list-files" });

	assert.equal(tool.name, "review_context");
	assert.equal(listFilesResult.isError, false);
	assert.match(listFilesResult.content[0]?.text ?? "", /src\/review\.ts.*available/);
});

test("review context tool fails clearly when no review context is active", async () => {
	const tool = createReviewContextTool(() => undefined, {
		name: "review_context",
		label: "Review Context",
		description: "Read-only shared review context.",
	});

	const result = await tool.execute("tool-missing", { kind: "list-files" });

	assert.equal(result.isError, true);
	assert.match(result.content[0]?.text ?? "", /No active review context/i);
});

test("review context tool searches allowed files with matching excerpts", async () => {
	const tool = createTestReviewContextTool(buildReviewContext({
		diffSnapshot: "diff snapshot",
		files: [
	...
	const searchResult = await tool.execute("tool-11", { kind: "search", query: "token", maxResults: 2 });
	const limitedSearchResult = await tool.execute("tool-12", { kind: "search", query: "bug", maxResults: 1 });
...
test("review context tool lists hunks and returns hunk excerpts", async () => {
	const tool = createTestReviewContextTool(buildReviewContext({
		diffSnapshot: "diff snapshot",
		files: [
			{ path: "src/auth.ts", content: "line1\nline2\nline3" },
	...
	const listHunksResult = await tool.execute("tool-14", { kind: "list-hunks", path: "src/auth.ts" });
	const hunkExcerptResult = await tool.execute("tool-15", { kind: "hunk-excerpt", path: "src/auth.ts", hunkId: "H2" });
...
test("review context tool rejects unknown or unsafe paths", async () => {
	const tool = createTestReviewContextTool(buildReviewContext({
		diffSnapshot: "diff snapshot",
		files: [
			{ path: "src/auth.ts", content: "line1\nline2\nline3" },
	...
	const unknown = await tool.execute("tool-7", { kind: "file", path: "src/missing.ts" });
	const unsafe = await tool.execute("tool-8", { kind: "file", path: "../secret.txt" });
	const unsafeDiff = await tool.execute("tool-9", { kind: "file-diff", path: "../secret.txt" });
	const unsafeExcerpt = await tool.execute("tool-10", { kind: "file-excerpt", path: "../secret.txt", startLine: 1, endLine: 2 });
	const emptySearch = await tool.execute("tool-13", { kind: "search", query: "", maxResults: 2 });
	const badHunk = await tool.execute("tool-16", { kind: "hunk-excerpt", path: "src/auth.ts", hunkId: "missing" });
